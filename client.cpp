...
void splitString(const char *input, char *username, char *filename) {
    ...
    if (i < inputLen) {
        // Calculate the lengths of the username and filename parts.
        size_t usernameLen = i;
        size_t filenameLen = inputLen - i - 2; // Subtract 2 for '->'
        
        // Copy the username and filename parts into the respective output buffers.
        strncpy(username, input, usernameLen);
        username[usernameLen] = '\0'; // Null-terminate the username string.
        strncpy(filename, input + i + 2, filenameLen);
        filename[filenameLen] = '\0'; // Null-terminate the filename string.
    } else {
        // No separator found, handle error or provide a default value.
        strcpy(username, "");
        strcpy(filename, "");
    }
}
...
string Blowfish_Decrypt(string ciphertextWithIV, byte key[], int keySize) {
    ...
    try {
        ...
        // Process the ciphertext without the IV
...
{
    // Create a ArraySink to store the output
    CryptoPP::ArraySink arraySink(output, length);

    // Use Put() to write the Integer to the ArraySink
...
//     // Concatenate the username and the message using strcat
...
if ((len = recv(my_sock, msg, 500, 0)) > 0) {
    msg[len] = '\0'; // Null-terminate the received message
